...
[![CRAN status](https://www.r-pkg.org/badges/version/cpam)](https://CRAN.R-project.org/package=cpam)
<!-- badges: end -->
...
![](man/figures/example_trends.png){width=800 height=600}
...
If you encounter issues or have suggestions for improvements, please open an [issue](https://github.com/l-a-yates/cpam/issues). We welcome questions and discussion about using **cpam** for your research through [Discussions](https://github.com/l-a-yates/cpam/discussions/). Our goal is to work with users to make **cpam** a robust and valuable tool for time series omics analysis. We can also be contacted via the email addresses listed in our paper [here](https://doi.org/10.1101/2024.12.22.630003). 
...
```
For the development version, you can install it from GitHub using the `remotes` package:
```{r, eval = FALSE}
...
In this *Arabidopsis thaliana* time series example, we used the software [kallisto](https://doi.org/10.1038/nbt.3519) to generate counts from RNA-seq data. To load the counts, we provide the file path for each kallisto output file (alternatively you can provide the counts directly as count matrix, or use other quantification software)
```{r, echo = TRUE, tibble.print_max = 10}
...
```
![](man/figures/example_gene_plot.png){width=650 height=550}
...
For a quick-to-run introductory example, we have provided a small simulated data 
